...
public:
	...
	UPROPERTY()
	ULoadScreenUserWidget* LoadScreen;

	UFUNCTION(Client, Reliable, Category = "Lobby")
	void ClientCreateLoadScreen();
	void ClientCreateLoadScreen_Implementation();

	UFUNCTION(Client, Reliable, Category = "Lobby")
	void UpdateLevelSelection(FName LevelRowName);
	void UpdateLevelSelection_Implementation(FName LevelRowName);
