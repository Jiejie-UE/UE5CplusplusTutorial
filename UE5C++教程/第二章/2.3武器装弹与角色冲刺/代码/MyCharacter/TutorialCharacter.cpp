...
void ATutorialCharacter::BindGunInputMappingContext()
{
	...
	if (APlayerController* PlayerController = Cast<APlayerController>(GetController())) {
		...
		if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerController->InputComponent)) {
			...
			EnhancedInputComponent->BindAction(GunReloadAction, ETriggerEvent::Triggered, this, &ATutorialCharacter::GunReloading);
		}
...
void ATutorialCharacter::Multicast_FireMontage_Implementation()
{
	if (MM_Pistol_Fire_Montage) {
		GetMesh()->GetAnimInstance()->Montage_Play(MM_Pistol_Fire_Montage, 1.0f, EMontagePlayReturnType::MontageLength, 0.0f, false);
		if (Weap_Pistol_Fire) {
...
void ATutorialCharacter::Multicast_ReloadMontage_Implementation()
{
	if (MM_Pistol_Reload_Montage) {
		float Diration = GetMesh()->GetAnimInstance()->Montage_Play(MM_Pistol_Reload_Montage, 1.0f, EMontagePlayReturnType::MontageLength, 0.0f, false);
...
{
	GetWorld()->GetTimerManager().SetTimer(Reset_IsReloadingTimerHandle, this, &ATutorialCharacter::Set_IsReloading_false, Diration, false);
}
