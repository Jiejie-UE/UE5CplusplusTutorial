	...
	if (MyGameInstance) {
		if (MyGameInstance->IsSoloGame) {
			...
			if (Num_ExpectedPlayer == All_ConnectedControllers.Num()) {
				...
				GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, *FString::Printf(TEXT("Test Using HandleStartingNewPlayer_Implementation，正常运行时请删除这里的逻辑")));
...
void ATutorialGameMode::LevelStartDelay1()
{
	...
	//这里的等待时间应该和InGameHUDUserWidget中的CountdownValue变量相同
